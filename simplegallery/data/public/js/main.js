...
function showCaption() {
  $("div.pswp__caption").css('opacity', 0.8);
}
function hideCaption() {
  $("div.pswp__caption").css('opacity', 0);
}
var hideCaptionTimeout = null;
function setupCaptionAutoHide() {
  // on desktop only:
  let caption = $(".pswp--has_mouse div.pswp__caption");
  if (caption.length > 0) {
    showCaption();
    caption.mouseenter(showCaption);
    caption.mouseleave(hideCaption);
    if (hideCaptionTimeout) {
      clearTimeout(hideCaptionTimeout);
      hideCaptionTimeout = null;
    }
    hideCaptionTimeout = setTimeout(hideCaption, 3000);//
  }
};
